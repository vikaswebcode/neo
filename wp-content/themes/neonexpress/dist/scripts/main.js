...
/******/ 	__webpack_require__.p = "/wp-content/themes/neonexpress/dist/";
/******/
...
/* WEBPACK VAR INJECTION */(function(jQuery) {/* harmony import */ var __WEBPACK_IMPORTED_MODULE_0_jquery_ui__ = __webpack_require__(3);
...
/* harmony import */ var __WEBPACK_IMPORTED_MODULE_1_dropzone_dist_dropzone_min__ = __webpack_require__(5);
/* harmony import */ var __WEBPACK_IMPORTED_MODULE_1_dropzone_dist_dropzone_min___default = __webpack_require__.n(__WEBPACK_IMPORTED_MODULE_1_dropzone_dist_dropzone_min__);
...
jQuery(document).ready(function ($) {
  ...
  $('.hero-scroll').click(function () {
    ...
    $('html, body').animate({
      scrollTop: heightHero - headerHeight
    }, 800);
  }); // CUSTOM QUOTE
  ...
  $('.contact-form-field-icons__up_quantity').on('click', function () {
    if ($('#quote-quantity').val().trim() != '') {
      var currentValue = parseInt($('#quote-quantity').val());
      currentValue += 1;
      ...
      if ($('#quote-quantity').val() === 1) {
        var _currentValue = parseInt($('#quote-quantity').val());

        _currentValue += 1;
        $('#quote-quantity').val(_currentValue);
      }
    }
  });
  ...
  var calcPrice = function calcPrice(height, width) {
    var basePrice = 100;
    var size = height * width;
    var price = basePrice + size * 0.1;
    return price;
  };
  ...
  var myDropzone = new Dropzone("#dropzone-previews", Dropzone.options.myDropzone);
  ...
  dropZone.on('drop', function (event) {
    event.preventDefault();
    $(this).removeClass('highlight');
    var files = event.originalEvent.dataTransfer.files; // Добавляем файлы в input

    var newInput = $('<input type="file" id="fileInput" name="uploaded_files[]" accept=".jpg, .jpeg, .png, .svg, .ai" multiple />');
    newInput[0].files = files; // Заменяем старый input на новый
  ...
  var sendQuote = function sendQuote() {
